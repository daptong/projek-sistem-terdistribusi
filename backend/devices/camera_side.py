"""Camera side device (event-only example)."""
import logging
from backend.core.threads import DeviceThread

logger = logging.getLogger("device.camera_side")

class CameraSideDevice(DeviceThread):
    device_type = "camera"
    ...
    def _on_mqtt_message(self, topic, payload):
        if topic == self.cmd_topic:
            logger.info("Camera side cmd received: %s", payload)
    ...
    def run(self):
        while not self.stopped():
            try:
                ...
                logger.exception("camera side status publish failed")
            self.sleep(30)
